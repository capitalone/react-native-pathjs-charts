...
export function toHex(color, level) {
    let colorArr = parse(color);
    if (colorArr) {
...
export function fastMapToColor(normalizedValue, colors, out = [0, 0, 0, 0]) {
    ...
    let rightColor = colorsCopy[rightIndex];
...
export function mapToColor(normalizedValue, colors, fullOutput) {
    if (!(colors && colors.length)
        || !(normalizedValue >= 0 && normalizedValue <= 1)
    ) {
        return;
